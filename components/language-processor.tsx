...
import { detectLanguage, correctText, translateText } from "@/lib/language-services"
...
export function LanguageProcessor() {
    ...
    try {
      ...
      for (const group of Object.keys(LanguageGroups)) {
        ...
        for (const lang of languages) {
          ...
          const literalTranslation = await translateText(translation.text, detected.code)
          ...
          groupTranslations[lang.code] = {
            ...
            alternativeTranslations: alternativeTranslations.map((t) => t.text).filter((t) => t !== translation.text), // Remove duplicates
          }
        }
    ...
    try {
      ...
      for (const group of Object.keys(LanguageGroups)) {
        ...
        for (const lang of languages) {
          ...
          // Get literal translation back to detected language
          const literalTranslation = await translateText(translation.text, languageCode)
  ...
  return (
    <div className="space-y-6">
      ...
      {detectedLanguages.length > 1 && !isProcessing && correctedText && (
        <DetectedLanguages
